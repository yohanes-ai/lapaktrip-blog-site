<?php
$rlogo = $limg = $lsitetitle = $lcustomtxt = $logohw = $logow_s = "";

$rlogo = get_post_meta( minimag_get_the_ID(), "minimag_cf_res_logo", true );
...
if( $rlogo != "" ) { 
	...
	<a class="image-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>">
		<img src="<?php echo esc_url( $rlogo ); ?>" alt=""<?php echo html_entity_decode( $logohw ); ?>/>
	</a>
	<?php
}
...
elseif( minimag_options('opt_logotype') == "1" && $lsitetitle != "" ) { // Logo - Site Title
	?>
	<a class="navbar-brand text-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_bloginfo( $lsitetitle ); ?></a>
	<?php
}
elseif( minimag_options('opt_logotype') == "3" && $lcustomtxt != "" ) { // Logo - Custom Text
	?>
	<a class="navbar-brand text-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>">
